...
restore_network()
{
    ...
    awk -F ';' '/^net/ { gsub(" ", ";", $5); if($3) print $2 " " $3 " " $4 " " $5; }' "$IGX_SYSCONF_FILE" | while read dev ip mask flags; do
        ...
        if [ $? -eq 0 ]; then
            ...
            igx_log "Adding device $dev to bonding_masters"
            echo "+$dev" > /sys/class/net/bonding_masters 2> /dev/null
            sleep 1
            ...
            ip link set $dev up > /dev/null 2> /dev/null
            if [ $? -ne 0 ]; then
                igx_log "ERROR: Cannot bring up bonding interface $dev without address, ABORT!"
                return 2
            else
                igx_log "Pre- Start of Bonding interface $dev without address successful"
                sleep 1
            fi
            ...
            if [ $? -ne 0 ]; then
                ...
                igx_log "Slave devices $slaves to bonding interface $dev successfully added!"
                sleep 1
            fi
        fi
        ...
        if [ $? -ne 0 ]; then
            igx_log "WARNING: Cannot configure device $dev with ip $ip/$mask!"
        else
            igx_log "Device $dev with ip $ip/$mask successfully configured!"
            ip link set $dev up > /dev/null 2> /dev/null
            if [ $? -ne 0 ]; then
                igx_log "WARNING: Cannot bring link up interface $dev, ABORT!"
            else
                igx_log "Link up on interface $dev successful!"
                sleep 1
            fi
        fi
    ...
    up_devs="$(ip link show | awk '/UP/ { gsub(":", "", $2); gsub("@.*", "", $2); print $2; }')"
    ...
    for dev in $up_devs; do
        ...
        awk -F ';' '/^route/ { 
            ...
            if("'$dev'" == route[5]) { 
                for(i = 1; i <= n; i++) 
                    printf("%s ", route[i]); printf("\n"); 
            } 
        }' "$IGX_SYSCONF_FILE" | while read route_arg; do
            ip route add $route_arg
            if [ $? -ne 0 ]; then
                igx_log "WARNING: Route \"$route_arg\" cannot install!"
            else
                igx_log "Route \"$route_arg\" successfully installed!"
            fi
        done
