...
# written in $IGX_CONFGSET_FILE: boot;<disk>;<boot dump file>;<fdisk file>
#
get_boot_disks()
{
    disk="$1"
    disk_masters="$(echo $disk | sed 's/[[:digit:]]*$//g')"
    disk_masters="$disk_masters $(echo $disk | sed 's/[a-z]*[[:digit:]]*$//g')"
    
    igx_verbose "Trying to get disks where boot sector is installed..."
    ...
    for mdisk in $disk_masters; do
        if [ -b "$mdisk" ]; then
            igx_verbose "Disk $disk has parent device $mdisk and will also checked for boot sector."
            disks="$disks $mdisk"
        fi
    done
    ...
    for bdev in $disks; do
        ...
        if [ $? -eq 0 ]; then
            ...
            echo "boot;$bdev;boot.$(basename $bdev);fdisk.$(basename $bdev)" >> $IGX_CONFGSET_FILE
...
# written in $IGX_CONFGSET_FILE: net;<dev>;<ip>;<netmask>;<extended flags> and route;<host/net>;<mask;<gateway>;<dev>
#
get_network()
{
    ...
    igx_verbose "Getting routing table for configured ethernet (eth) and bonding (bond) devices"
    route -n | awk '/^[[:digit:]].*eth/ { printf("route;%s;%s;%s;%s\n", $1, $3, $2, $NF); }' >> $IGX_CONFGSET_FILE
    route -n | awk '/^[[:digit:]].*bond/ { printf("route;%s;%s;%s;%s\n", $1, $3, $2, $NF); }' >> $IGX_CONFGSET_FILE
    if [ $? -ne 0 ]; then
        igx_log "ERROR: Cannot request routing table while using route -n command, ABORT!"
        return 1
    else
...
get_kernmod()
{
    ...
    lsmod | sort -n -k 3 | awk '!/^Module/ { print "module;"$1; }' >> $IGX_CONFGSET_FILE
    return $?
}
