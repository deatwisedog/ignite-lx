...
menu_edit_network()
{
    ...
    while read kind dev ip mask flags; do
        ...
        dialog_args=''$dialog_args'
                     ...
                     Flags  '$y' 55 '$(echo $flags | sed 's/ /_/g')' '$y' 61 11 40'
    ...
    while read item; do
        ...
        case $obj in
            ...
            *)
                ...
                if [ $? -ne 0 ]; then
                    values_ok=0
                    igx_menu_errmsg "Addr: $item not valid!"
                    break
                fi
                printf ";$item"
            ;;
...
menu_edit_routes()
{
    ...
    IFS=";"
    while read kind net mask gw dev; do
        
        if [ "$kind" != "route" ]; then
            continue
        fi
        
        if [ -z "$net" -o -z "$mask" -o -z "$gw" -o z "$dev" ]; then
            continue
        fi
        
        dialog_args=''$dialog_args'
                     Net  '$y'  2 '$net'  '$y'  6 16 0
                     Mask '$y' 23 '$mask' '$y' 28 16 0
                     Gw   '$y' 45 '$gw'   '$y' 48 16 0
                     Dev  '$y' 65 '$dev'  '$y' 69  5 6'
    ...
    while read item; do
        ...
        case $obj in
            ...
            4)
                obj=0
                echo "$item" | egrep '^eth|^bond' > /dev/null
                if [ $? -ne 0 ]; then
                    values_ok=0
                    igx_menu_errmsg "Device: $item not valid!"
                    break
                fi
                printf ";$item\n"
            ;;
